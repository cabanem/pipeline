    ...
    build_parameterized_url: {
      ...
      input_fields: lambda do |_object_definitions, _connection, _config_fields|
        [
...

          {
            ...
            label: 'Source Fields',
            optional: true,
            hint: 'Key/value pairs used by mappings.',
            properties: [
              { name: 'key', type: :string, label: 'Key', control_type: 'text', optional: false, sticky: true },
              { name: 'value', type: :string, label: 'Value', control_type: 'text', optional: true, sticky: true }
            ]
...

          {
            ...
            optional: true,
            hint: 'Direct payload fields (bypass mappings).',
            properties: [
              { name: 'key', type: :string, label: 'Key', control_type: 'text', optional: false, sticky: true },
              { name: 'value', type: :string, label: 'Value', control_type: 'text', optional: true, sticky: true },
              { name: 'disabled', type: :boolean, label: 'Disabled?', control_type: 'checkbox', optional: true, default: false }
...

          {
            ...
            label: 'Entries',
            optional: true,
            hint: 'Optional entries array used by indexed_entry_mappings.',
            properties: [
              { name: 'fields', type: :object, label: 'Fields', control_type: 'text', optional: true }
            ]
          }
        ]
      end,
      ...
      execute: lambda do |_connection, input|
        ...
        mappings.each do |m|
          ...
          if !!m['required'] && !call(:present?, val)
            raise "Missing required mapped field: #{in_key}"
          end
        ...
        # 3) Direct prefill fields (override)

...

{
    ...
    gen_generate: {
      ...
      execute: lambda do |connection, raw_input|
        ...
        begin
          ...
          # Make request and ensure response is parsed
  ...
  methods: {
    ...
    gen_generate_process_response!: lambda do |resp, input, request_info, connection|
      # Now we can assume resp is a Hash because we parsed it in execute
      req  = request_info.is_a?(Hash) ? request_info : {}
      t0   = req['started_at'] || Time.now  # Use req, not request_info
      corr = req['correlation_id']           # Use req, not request_info

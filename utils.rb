...

{
  ...
  methods: {
    ...
    process_email_text: lambda do |input|
      raw = (input['email_body'] || '').to_s
      original_length = raw.length

      # Optional HTML → text
      if input['strip_html'] != false
        # Heuristic: if it looks like HTML or strip_html explicitly true, sanitize
        looks_html = raw.include?('<') && raw.include?('>')
        raw = call(:util_html_to_text, nil, raw) if looks_html || input['strip_html'] == true
      end

      # Unicode noise
      cleaned = input['strip_zero_width'] == false ? raw.dup : call(:util_strip_unicode_noise, nil, raw)
      ...
      if input['remove_quotes'] != false
        # Detect common header markers; cut everything from the first marker onward
        header_idx = lines.find_index do |l|
          l =~ /^\s*>/ ||                                     # traditional > quote
          l =~ /^On .+ wrote:\s*$/i ||                        # "On Mon, … wrote:"
          l =~ /^-----Original Message-----\s*$/i ||          # Outlook
          l =~ /^\s*From:\s.+/i &&                            # Outlook header block start
              (lines[lines.index(l), 6] || []).any? { |h| h =~ /^\s*(Sent|To|Subject):/i }
        end

        if header_idx
          removed_sections << lines[header_idx..-1].join
          lines = lines[0...header_idx]
        else
          # Fallback: drop only leading '>' lines, preserving inline '>'
          quoted = lines.select { |l| l.lstrip.start_with?('>') }
          unless quoted.empty?
            removed_sections << quoted.join
            lines = lines.reject { |l| l.lstrip.start_with?('>') }
          end
        end
      end

      # -------- Signature detection (tail-aware) ----------
      if input['remove_signatures'] != false
        window = (input['signature_tail_window'] || 40).to_i
        window = 0 if window < 0
        start_tail = [lines.length - window, 0].max
        sig_region = lines[start_tail..-1] || []

        sig_idx_rel = sig_region.rindex do |l|
          l =~ /^\s*(--\s*$|—\s*$)/i ||                       # RFC delimiter or em-dash on its own line
          l =~ /^\s*(Best regards|Regards|Sincerely|Thanks|Kind regards|Cheers)[,–-]?\s*$/i ||
          l =~ /^\s*Sent from my/i
        end

        if sig_idx_rel
          sig_idx = start_tail + sig_idx_rel
          removed_sections << lines[sig_idx..-1].join
          lines = lines[0...sig_idx]
        end
      end

      # -------- Disclaimers (tail-aware) ----------
      if input['remove_disclaimers'] != false
        tail = lines[[lines.length - 80, 0].max..-1] || []
        disc_idx_rel = tail.rindex do |l|
          l =~ /(confidentiality notice|confidential|privileged|intended only for the (addressee|recipient)|unauthorized (review|disclosure))/i ||
          l =~ /(This (e-)?mail|This message).*(confidential|intended only)/i
        end
        if disc_idx_rel
          disc_idx = [lines.length - tail.length, 0].max + disc_idx_rel
          removed_sections << lines[disc_idx..-1].join
          lines = lines[0...disc_idx]
        end
      end

      cleaned = lines.join

      # -------- URLs (after structural removals) ----------
      if input['extract_urls']
        extracted_urls = cleaned.scan(%r{https?://[^\s<>"'()]+})
      end

      # -------- Whitespace normalization ----------
      if input['normalize_whitespace'] != false
        cleaned.gsub!(/[ \t]+/, ' ')
        cleaned.gsub!(/\n{3,}/, "\n\n")
        cleaned.strip!
      end

      # Extract a primary query paragraph
      extracted_query =
        cleaned.split(/\n{2,}/).find { |p| p.strip.length.positive? } ||
        cleaned[0, 200].to_s

      has_content = cleaned.strip.length > 10
...

      {
        ...
        reduction_percentage: (original_length.zero? ? 0.0 : ((1 - cleaned.length.to_f / original_length) * 100)).round(2),
    ...
    util_html_to_text: lambda do |_connection, html|
      s = (html || '').to_s
      ...
      s = s.gsub(/<\/(p|div|h\d|li|br|tr)>/i, "\n")

      # Replace <li> with bullet-ish prefix
      s = s.gsub(/<li[^>]*>/i, "• ")
    ...
    util_strip_unicode_noise: lambda do |_connection, text|
      t = (text || '').to_s
      return t if t.empty?
      t = t.gsub("\u00A0", ' ')              # NBSP → space
  ...
  object_definitions: {
    ...
    email_cleaning_options: {
      fields: lambda do
        [
          { name: "strip_html",             label: "Strip HTML (if present)", type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "remove_signatures",      label: "Remove signatures",       type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "signature_tail_window",  label: "Signature tail window (lines)", type: "integer", default: 40,
            convert_input: "integer_conversion", sticky: true, group: "Options",
            hint: "Only look for signatures within the last N lines." },
          { name: "remove_quotes",          label: "Remove quoted text",      type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "remove_disclaimers",     label: "Remove disclaimers",      type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "normalize_whitespace",   label: "Normalize whitespace",    type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "strip_zero_width",       label: "Strip zero-width chars",  type: "boolean", default: true,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" },
          { name: "extract_urls",           label: "Extract URLs",            type: "boolean", default: false,
            control_type: "checkbox", convert_input: "boolean_conversion", sticky: true, group: "Options" }
        ]
      end
    },
